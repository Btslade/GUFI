...
def add_to_machine_table(hash, args):
    ...
    if table == []:
        create_table_str = "'hash', 'cpu', 'ram', 'machine_name', 'hash_type',  PRIMARY KEY (hash)"
        cur.execute(f"CREATE TABLE machine ({create_table_str});")
    # https://softhints.com/python-3-convert-dictionary-to-sql-insert/
    
    sql = f"INSERT INTO machine ('hash', 'cpu', 'ram', 'machine_name', 'hash_type') VALUES ('{hash}', '{args.cpu}', '{args.ram}', '{args.machine_name}', '{args.hash}');"
...
def add_to_gufi_command_table(hash, args):
    ...
    if table == []:
        create_table_str = "'hash', 'gufi', '-S', '-E', 'tree', 'hash_type',  PRIMARY KEY (hash)"
        cur.execute(f"CREATE TABLE gufi_command ({create_table_str});")
    # https://softhints.com/python-3-convert-dictionary-to-sql-insert/
    
    sql = f"INSERT INTO gufi_command ('hash', 'gufi', '-S', '-E', 'tree', 'hash_type') VALUES ('{hash}', '{args.gufi_command}', '{args.S}', '{args.E}', '{args.tree}', '{args.hash}');"
...
def add_to_full_hash_table(hash, args):
    ...
    if table == []:
        create_table_str = "'combined_hash', 'machine_hash', 'gufi_hash', 'hash_type',  PRIMARY KEY (combined_hash)"
        cur.execute(f"CREATE TABLE full_hash ({create_table_str});")
    # https://softhints.com/python-3-convert-dictionary-to-sql-insert/
    
    sql = f"INSERT INTO full_hash ('combined_hash', 'machine_hash', 'gufi_hash', 'hash_type') VALUES ('{hash}', '{args.machine_hash}', '{args.gufi_hash}',  '{args.hash}');"
