...
def add_to_machine_table(con, hash, args):
    con.execute(f'INSERT INTO {args.table} (hash, cpu, ram, machine_name, hash_type, notes) VALUES ("{hash}", "{args.cpu}", "{args.ram}", "{args.machine_name}", "{args.hash}", "{args.notes}");')

def add_to_gufi_command_table(con, hash, args):
    con.execute(f'INSERT INTO {args.table} (hash, gufi, S, E, tree, hash_type, notes) VALUES ("{hash}", "{args.gufi_command}", "{args.S}", "{args.E}", "{args.tree}", "{args.hash}", "{args.notes}");')

def add_to_full_hash_table(con, full_hash, args):
    con.execute(f'INSERT INTO {args.table} ({COMBINED_HASH_COL}, machine_hash, gufi_hash, hash_type, notes) VALUES ("{full_hash}", "{args.machine_hash}", "{args.gufi_hash}", "{args.hash}", "{args.notes}");')
